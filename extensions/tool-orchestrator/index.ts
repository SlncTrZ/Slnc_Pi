...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Search Tools",
		description:
			"Search for and activate tools relevant to your current task. Call this when you need capabilities beyond the currently available tools.",
		promptSnippet:
			"Use search_tools to find and activate additional tools for your task",
		promptGuidelines: [
		...
		parameters: Type.Object({
			query: Type.String({
				description: "Describe the capability or task you need tools for",
			}),
		}),
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			if (matchedGroups.length === 0) {
				const available = Object.keys(TOOL_GROUPS)
					.filter((g) => g !== "core")
					.join(", ");
				return {
					content: [
						{
							type: "text",
							text: `No relevant tool groups found for: "${params.query}". Available groups: ${available}`,
						},
					],
					details: { matches: [] },
				};
	...
	pi.registerTool({
		...
		description:
			"Deactivate previously loaded tool groups to reduce context overhead. Specify which groups to remove.",
		parameters: Type.Object({
			groups: Type.Optional(
				Type.Array(
					Type.String({
						description:
							"Group names: code-intel, diagnostics, web, lean-ctx, orchestration, pi-core, goal",
					}),
				),
			),
			all: Type.Optional(
				Type.Boolean({
					description: "Set true to deactivate all non-core groups",
				}),
			),
		}),
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			if (params.all) {
				...
				toDeactivate = getToolsForGroups(
					params.groups.filter((g) => ACTIVE_GROUP_CACHE.has(g)),
				);
			} else {
				return {
					content: [
						{
							type: "text",
							text: "Specify groups to deactivate, or set all: true",
						},
					],
					details: {},
				};
			...
			const otherTools = currentActive.filter(
				(t) =>
					!toDeactivate.includes(t) &&
					!CORE_TOOLS.includes(t) &&
					!LOADER_TOOLS.includes(t) &&
					!NON_CORE_TOOLS.has(t),
			);
			...
			return {
				content: [
					{
						type: "text",
						text: `✅ Deactivated ${toDeactivate.length} tools. Core + loaders remain.`,
					},
				],
	...
	pi.registerCommand("orchestrate", {
		description:
			"Manually control tool groups: /orchestrate activate <group1,group2> | deactivate <group1,group2> | status",
		handler: async (args, ctx) => {
			const parts = (args ?? "").trim().split(/\s+/);
			if (parts.length < 2) {
				ctx.ui.notify(
					"Usage: /orchestrate activate|deactivate|status [groups]",
					"info",
				);
				return;
			}
			...
			if (command === "status") {
				...
				ctx.ui.notify(
					`Groups: ${groups.join(", ") || "none"}\nTools: ${activeTools.length} active`,
					"info",
				);
				return;
			}
			...
			if (command === "activate") {
				...
				ctx.ui.notify(
					`Activated ${groupNames.join(", ")} (${added.length} tools)`,
					"info",
				);
				return;
			}
			...
			ctx.ui.notify(
				`Unknown: ${command}. Use: activate, deactivate, status`,
				"warning",
			);
		},
	});
	...
	pi.on("session_start", async () => {
		...
		pi.setActiveTools([
			...new Set([...CORE_TOOLS, ...LOADER_TOOLS, ...otherExtTools]),
		]);
