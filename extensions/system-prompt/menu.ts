...
type MenuInputItem = Extract<MenuItem, { type: "input" }>;
...
/**
 * Open a small tree menu. Stays alive until the user presses Escape at the
 * root level. Calls `onAction(id, value?)` for leaf actions and input
 * submissions. After the callback resolves, the menu re-renders via
 * `treeFactory`.
 */
export async function openMenu(
  ...
  await ctx.ui.custom<void>((tui, theme, _kb, done) => {
    ...
    let inputBuffer = "";
    let inputItem: MenuInputItem | null = null;
