...
function trimHistory(msgs: SimpleMessage[], max: number): SimpleMessage[] {
	...
	const nonsys = msgs.filter(
		(m) => m.role !== "system" && m.role !== "developer",
	);

	// Group into atomic blocks — tool_calls + their tool responses are inseparable
	const blocks: SimpleMessage[][] = [];
	let currentBlock: SimpleMessage[] = [];
	...
	for (const msg of nonsys) {
		...
		if (msg.role === "assistant" && Array.isArray(msg.content)) {
			for (const block of msg.content) {
				if ((block as any)?.type === "toolCall" && (block as any)?.id) {
					pendingCallIds.add((block as any).id);
				}
			}
		}

		if (msg.role === "tool" && (msg as any).toolCallId) {
			pendingCallIds.delete((msg as any).toolCallId);
		}
	...
	if (blocks.length <= keep) return msgs;

	const first = Math.floor(keep * 0.3);
	const last = keep - first;
	const keptBlocks = [...blocks.slice(0, first), ...blocks.slice(-last)];
	const removedCount = nonsys.length - keptBlocks.reduce((s, b) => s + b.length, 0);
	const note: SimpleMessage = {
		role: "system",
		content: `[Context pruned: ${removedCount} messages removed. Earlier conversation condensed.]`,
	};

	return [sys, note, ...keptBlocks.flat()].flat();
}
	...
	(
		pi as unknown as {
			on: (
				...
				) => void | Promise<Record<string, unknown> | undefined>,
			) => void;
		}
		...
		const step1 = messages.map((msg) => {
			...
			return {
				...msg,
				content: truncateContent(msg.content, MAX_MESSAGE_CHARS),
			};
		});

		// 2. Suppress redundant diagnostics — replace content with placeholder, NEVER remove entire message
		// (removing a tool message would break the tool_calls → tool response chain, causing API 400)
		const step2 = step1.map((msg) => {
			if (msg.role !== "tool") return msg;
			if (!hasDiagnostics(msg)) return msg;
			const text = typeof msg.content === "string" ? msg.content : "";
			const lines = text.split("\n");
			const allKnown = lines.every((line) => {
				if (!line.trim()) return true;
				if (!line.includes(":") || !/\d+/.test(line)) return true;
			...
			if (allKnown) {
				suppressedCount++;
				changed = true;
				return { ...msg, content: "[diagnostics suppressed]" };
			}
			return msg;
		});
