...
For this vendored extension, use `/mcp setup` inside pi for guided setup. The upstream `pi-mcp-adapter init` CLI is only relevant when installing `pi-mcp-adapter` as a standalone npm package.
...
When using this vendored extension, prefer `/mcp setup` inside pi to detect host-specific configs and add missing imports to the Pi agent dir config.
