...
Bản chuẩn đồng bộ từ `Slnc_Pi/pi-config/AGENTS.md` (2026-08-12) — mọi sửa đổi phải qua repo rồi sync.
...
**Khi làm việc liên quan server .227, docker, deployment → PHẢI đọc Qdrant .227:6333 trước (hoặc tra KB qua MCP meilin-brain):**
...
**Quick Reference (đã cập nhật 2026-08-12 — kiểm tra thực tế qua SSH, KHÔNG tin số liệu cũ):**
...
4. **Wiki-First khi Search Web (BẮT BUỘC):** Khi Anh yêu cầu nghiên cứu/search thông tin → đầu tiên tra KB domain `research` (score ≥ 0.7) → nếu có → trả lời trực tiếp, không web search. Nếu không có → `web_search` → tổng hợp → trả lời → lưu vào KB domain `research`.
