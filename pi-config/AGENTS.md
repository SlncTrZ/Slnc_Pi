...
- **CUỐI MỖI PHIÊN (BẮT BUỘC 2 bước, đúng thứ tự):**
  1. `meilin_brain_conversation_save` (hoặc canonical `meilin_brain_memory_store` với `session_id` + `event_time`) → `cyberbrain_episodic`
  2. `meilin_brain_dream_enqueue` → tạo **queue Dreaming/Reasoning** để hệ thống bên dưới tự suy tưởng (evidence-gated). PHẢI gọi sau `conversation_save`.
- ⚠️ KHÔNG viết node script / REST thủ công để log KB — MCP đã xử lý embedding sẵn
- **Compact-first recall:** `meilin_brain_knowledge_search` / `meilin_brain_memory_search` mặc định `view=compact` (trả `recall_text` rút gọn + `content_omitted=true`, `content`/`summary` bị ẩn). Cần full → lấy 1 ID trả về rồi gọi `meilin_brain_knowledge_get` / `meilin_brain_memory_get` (fetch exact, không search lại). `view=full` chỉ khi thật sự cần nguyên row.
...
- **Tool set meilin-brain = CyberBrain** (server `https://meilin-mcp.truongcongdinh.org/mcp`; tool thực gọi qua gateway có tiền tố `meilin_brain_`). **KHÔNG hardcode version / schema_version / contract_version / contract_hash / tool-count vào tài liệu** — các giá trị đó PHẢI đọc từ `meilin_brain_help` ở đầu phiên; danh sách dưới đây chỉ minh hoạ cấu trúc và có thể lệch theo version:
...
- Chạy `meilin_brain_help` trước để đọc contract/version/hash hiện hành.
- Tra cứu: `meilin_brain_knowledge_search` | `meilin_brain_memory_search` (canonical) | `meilin_brain_ai_memory_read` (legacy combined)
